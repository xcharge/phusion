...
public class CiphertextTransformer {
    ...
    public static JSONObject translateRequestData(String data, String operatorId, String secretKey, String secretIv, String sigSecret) throws Exception {
        JSONObject param = new JSONObject();

        param.put("OperatorID", operatorId);

        String encrypt = CommonUtil.encrypt(data, secretKey, secretIv);
        ...
        String hmacMd5Str = CommonUtil.sign(value, sigSecret);
        param.put("Sig", hmacMd5Str);

        return param;
    }

    public static JSONObject translateResponseData(String data, String secretKey, String secretIv) {
        JSONObject decData = null;
        try {
            decData = JSONObject.parseObject(CommonUtil.decrypt(data, secretKey, secretIv));
        } catch (Exception e) {
        }
