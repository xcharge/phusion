 ...
 */
public class StaticTraffic extends HttpBaseApplication {
    private final static String _position = StaticTraffic.class.getName();
    ...
    public DataObject requestParkingDiscount(DataObject msg, String integrationId, String connectionId, Context ctx) throws Exception {
        ...
        String key = getConnectionConfig(connectionId).getString("key");

        JSONObject objMsg;
        try {
            objMsg = Transformer.translateRequestMessage(msg.getJSONObject(), key);
        } catch (Exception ex) {
